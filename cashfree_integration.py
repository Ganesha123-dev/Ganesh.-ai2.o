...
from main import app, db, User, Transaction, PaymentOrder, WithdrawalRequest, log
...
class CashfreePaymentSystem:
    ...
    def __init__(self):
        ...
        self.app_id = os.getenv('CASHFREE_CLIENT_ID')
        self.secret_key = os.getenv('CASHFREE_CLIENT_SECRET')
    ...
    def process_successful_payment(self, payment_order, webhook_data: Dict):
        ...
        try:
            ...
            # Check if payment already processed to prevent double crediting
            existing_transaction = Transaction.query.filter_by(
                user_id=user.id,
                payment_id=payment_order.order_id
            ).first()
            if existing_transaction:
                log("payment", "WARNING", f"Payment {payment_order.order_id} already processed, skipping credit")
                return
            
            # Process based on payment purpose
            if purpose == 'wallet_topup':
                # Add money to wallet with payment_id for idempotency
                user.add_earnings(amount, f"Wallet top-up via Cashfree", payment_id=payment_order.order_id)
            ...
            # No need to create separate transaction - user.add_earnings already creates it with payment_id
        ...
        try:
            with app.app_context():
                ...
                for txn in transactions:
                    history.append({
                        'id': txn.payment_id or f"T{txn.id}",

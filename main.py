...
class User(db.Model):
    ...
    def generate_referral_code(self):
        ...
        if not self.referral_code and self.id:
            ...
            # Generate temporary code if ID is not available yet
            import uuid
            self.referral_code = f"GANESH{uuid.uuid4().hex[:8].upper()}"
    
    def add_earnings(self, amount, description="", payment_id=None):
    ...
    try:
        with app.app_context():
            ...
            if not admin_user:
                ...
                admin_user.email = BUSINESS_EMAIL
                admin_user.role = 'admin'
                admin_user.wallet = 1000.0  # Give admin some initial credits
        ...
        try:
            ...
            if response['success']:
                ...
                if user and model_key != 'free' and not user.is_premium():
                    ...
                    # Credit admin share to admin user
                    admin_user = User.query.filter_by(role='admin').first()
                    if admin_user:
                        admin_user.add_earnings(admin_earnings, f"Admin share from {model['name']} chat")
                    
                    # Credit user share back to user as reward
                    user.add_earnings(user_earnings, f"Chat reward from {model['name']}")
                    
                    # Record API usage
                    usage = APIUsage()
                    usage.user_id = user.id
                    usage.api_type = model_key
                    usage.model_name = model['name']
                    usage.cost = model['cost']
                    usage.earnings_generated = admin_earnings
                    usage.request_data = prompt[:500]
                    usage.response_data = response['content'][:500]
    ...
    async def _openai_request(self, prompt: str, model: str):
        ...
        try:
            ...
            if not HTTPX_AVAILABLE or httpx is None:
                return {'success': False, 'error': 'HTTP client library not available. Please install httpx.'}
    ...
    async def _huggingface_request(self, prompt: str, model: str):
        ...
        try:
            ...
            if not HTTPX_AVAILABLE or httpx is None:
                ...
                responses = [
                    f"Hello! I'm Ganesh AI. You asked: '{prompt[:50]}...' - For full functionality, please ensure all dependencies are installed.",
                    f"Thanks for using Ganesh AI! Regarding '{prompt[:50]}...', I can provide basic responses.",
                ]
                return {'success': True, 'content': random.choice(responses)}
...
def track_visit(user_id=None, page='/', referrer=None):
    ...
    try:
        ...
        visit.earnings_generated = VISIT_PAY_RATE
...
def api_chat():
    ...
    try:
        ...
        track_api_usage(user.id, model, message, response)
...
def track_api_usage(user_id, model, prompt, response):
    """Track API usage for monetization and analytics"""
    try:
        # Get model costs
        model_costs = {
            'ganesh-free': FREE_COST,
            'gpt-4-turbo': GPT4_COST,
            'claude-3-sonnet': CLAUDE_COST,
            'gemini-pro': GEMINI_COST
        }
        
        cost = model_costs.get(model, FREE_COST)
        
        usage = APIUsage()
        usage.user_id = user_id
        usage.api_type = model
        usage.model_name = model
        usage.cost = cost
        usage.earnings_generated = cost * ADMIN_SHARE
        usage.request_data = prompt[:500]
        usage.response_data = response[:500]
        
        db.session.add(usage)
        db.session.commit()
        
        log("api", "INFO", f"API usage tracked for user {user_id}, model {model}, cost ₹{cost}")
        
    except Exception as e:
        log("api", "ERROR", f"Failed to track API usage: {e}")
...
def generate_ai_response(message, model, user):
    ...
    try:
        ...
        # For paid models, use sync wrapper to call async AI manager
        import asyncio
        try:
            # Create a new event loop for this thread if none exists
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            result = loop.run_until_complete(ai_manager.generate_response(message, model, user))
            loop.close()
            return result.get('content', 'AI response unavailable')
        except Exception as e:
            log("ai", "ERROR", f"Async AI call failed: {e}")
            return generate_free_response(message, user)
...
def get_payment_history_api():
    ...
    try:
        ...
        history = get_transaction_history(session['user_id'], limit=20)
        return jsonify({'success': True, 'history': history})
        
    except Exception as e:
        log("payment", "ERROR", f"Payment history error: {e}")
        return jsonify({'success': False, 'message': 'Failed to load payment history'})
